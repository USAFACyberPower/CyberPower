...
class App(QWidget):
    def __init__(self):
        ...
        self.phaseAC_btn_reset.clicked.connect(self.menu_btn_reset_clicked)
        self.phaseAC_btn_relay1.clicked.connect(self.phaseAC_btn_relay1_clicked)
        self.phaseAC_btn_relay2.clicked.connect(self.phaseAC_btn_relay2_clicked)
        self.phaseAC_btn_relay3.clicked.connect(self.phaseAC_btn_relay3_clicked)
        self.phaseAC_btn_relay4.clicked.connect(self.phaseAC_btn_relay4_clicked)
        self.phaseAC_btn_relay5.clicked.connect(self.phaseAC_btn_relay5_clicked)
        self.phaseAC_btn_relay6.clicked.connect(self.phaseAC_btn_relay6_clicked)
        self.phaseAC_btn_relay7.clicked.connect(self.phaseAC_btn_relay7_clicked)
        self.phaseAC_btn_relay8.clicked.connect(self.phaseAC_btn_relay8_clicked)
        ...
        self.phaseBC_btn_reset.clicked.connect(self.menu_btn_reset_clicked)
        self.phaseBC_btn_relay1.clicked.connect(self.phaseBC_btn_relay1_clicked)
        self.phaseBC_btn_relay2.clicked.connect(self.phaseBC_btn_relay2_clicked)
        self.phaseBC_btn_relay3.clicked.connect(self.phaseBC_btn_relay3_clicked)
        self.phaseBC_btn_relay4.clicked.connect(self.phaseBC_btn_relay4_clicked)
        self.phaseBC_btn_relay5.clicked.connect(self.phaseBC_btn_relay5_clicked)
        self.phaseBC_btn_relay6.clicked.connect(self.phaseBC_btn_relay6_clicked)
        self.phaseBC_btn_relay7.clicked.connect(self.phaseBC_btn_relay7_clicked)
        self.phaseBC_btn_relay8.clicked.connect(self.phaseBC_btn_relay8_clicked)
    ...
    def phaseAB_btn_relay8_clicked(self):
        ...
        serial_transmit(tx)
    # END Phase A/B Definitions

    # BEGIN Phase A/C Definitions
    def phaseAC_btn_relay1_clicked(self):
        # Turn on relay 1 for bank 1
        tx = [170, 3, 254, 108, 1, 24]
        serial_transmit(tx)
        # Turn on relay 1 for bank 4
        tx = [170, 3, 254, 108, 4, 27]
        serial_transmit(tx)

    def phaseAC_btn_relay2_clicked(self):
        # Turn on relay 2 for bank 1
        tx = [170, 3, 254, 109, 1, 25]
        serial_transmit(tx)
        # Turn on relay 2 for bank 4
        tx = [170, 3, 254, 109, 4, 28]
        serial_transmit(tx)

    def phaseAC_btn_relay3_clicked(self):
        # Turn on relay 3 for bank 1
        tx = [170, 3, 254, 110, 1, 26]
        serial_transmit(tx)
        # Turn on relay 3 for bank 4
        tx = [170, 3, 254, 110, 4, 29]
        serial_transmit(tx)

    def phaseAC_btn_relay4_clicked(self):
        # Turn on relay 4 for bank 1
        tx = [170, 3, 254, 111, 1, 27]
        serial_transmit(tx)
        # Turn on relay 4 for bank 4
        tx = [170, 3, 254, 111, 4, 30]
        serial_transmit(tx)

    def phaseAC_btn_relay5_clicked(self):
        # Turn on relay 5 for bank 1
        tx = [170, 3, 254, 112, 1, 28]
        serial_transmit(tx)
        # Turn on relay 5 for bank 4
        tx = [170, 3, 254, 112, 4, 31]
        serial_transmit(tx)

    def phaseAC_btn_relay6_clicked(self):
        # Turn on relay 6 for bank 1
        tx = [170, 3, 254, 113, 1, 29]
        serial_transmit(tx)
        # Turn on relay 6 for bank 4
        tx = [170, 3, 254, 113, 4, 32]
        serial_transmit(tx)

    def phaseAC_btn_relay7_clicked(self):
        # Turn on relay 7 for bank 1
        tx = [170, 3, 254, 114, 1, 30]
        serial_transmit(tx)
        # Turn on relay 7 for bank 4
        tx = [170, 3, 254, 114, 4, 33]
        serial_transmit(tx)

    def phaseAC_btn_relay8_clicked(self):
        # Turn on relay 8 for bank 1
        tx = [170, 3, 254, 115, 1, 31]
        serial_transmit(tx)
        # Turn on relay 8 for bank 4
        tx = [170, 3, 254, 115, 4, 34]
        serial_transmit(tx)
    # END Phase A/C Definitions

    # BEGIN Phase B/C Definitions
    def phaseBC_btn_relay1_clicked(self):
        # Turn on relay 1 for bank 3
        tx = [170, 3, 254, 108, 3, 26]
        serial_transmit(tx)
        # Turn on relay 1 for bank 4
    ...
    def phaseBC_btn_relay2_clicked(self):
        # Turn on relay 2 for bank 3
        tx = [170, 3, 254, 109, 3, 27]
        serial_transmit(tx)
        # Turn on relay 2 for bank 4
    ...
    def phaseBC_btn_relay3_clicked(self):
        # Turn on relay 3 for bank 3
        tx = [170, 3, 254, 110, 3, 28]
        serial_transmit(tx)
        # Turn on relay 3 for bank 4
    ...
    def phaseBC_btn_relay4_clicked(self):
        # Turn on relay 4 for bank 3
        tx = [170, 3, 254, 111, 3, 29]
        serial_transmit(tx)
        # Turn on relay 4 for bank 4
    ...
    def phaseBC_btn_relay5_clicked(self):
        # Turn on relay 5 for bank 3
        tx = [170, 3, 254, 112, 3, 30]
        serial_transmit(tx)
        # Turn on relay 5 for bank 4
    ...
    def phaseBC_btn_relay6_clicked(self):
        # Turn on relay 6 for bank 3
        tx = [170, 3, 254, 113, 3, 31]
        serial_transmit(tx)
        # Turn on relay 6 for bank 4
    ...
    def phaseBC_btn_relay7_clicked(self):
        # Turn on relay 7 for bank 3
        tx = [170, 3, 254, 114, 3, 32]
        serial_transmit(tx)
        # Turn on relay 7 for bank 4
    ...
    def phaseBC_btn_relay8_clicked(self):
        # Turn on relay 8 for bank 3
        tx = [170, 3, 254, 115, 3, 33]
        serial_transmit(tx)
        # Turn on relay 8 for bank 4
